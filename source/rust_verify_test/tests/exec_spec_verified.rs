...
test_verify_one_file! {
    /// `e is Variant` used inside an impl method body, mixed with `&&&` and
    /// a match. Mirrors the auth-spec `ConditionalKey::is_valid` shape.
    #[test] test_exec_spec_is_in_impl IMPORTS.to_string() + verus_code_str! {
        exec_spec_verified! {
            pub enum KeyType {
                String_,
                Integer,
            ...
            pub enum ConditionalOperator {
                IsPresent,
                IsEqualToString { string_value: SpecString },
                IsEqualToInt { int_value: i32 },
                IsEqualToBool { bool_value: bool },
                ContainsValueString { string_value: SpecString },
                ContainsValueInt { int_value: i32 },
                ContainsValueBool { bool_value: bool },
            }
            ...
            impl ConditionalKey {
                pub open spec fn is_valid(&self) -> bool {
                    &&& self.key.len() > 0
                    &&& match self.operator {
                        ConditionalOperator::IsPresent => true,
                        ConditionalOperator::IsEqualToString { .. } => self.key_type is String_,
                        ConditionalOperator::IsEqualToInt { .. } => self.key_type is Integer,
                        ConditionalOperator::IsEqualToBool { .. } => self.key_type is Bool_,
                        ConditionalOperator::ContainsValueString { .. } => self.key_type is StringList,
                        ConditionalOperator::ContainsValueInt { .. } => self.key_type is IntegerList,
                        ConditionalOperator::ContainsValueBool { .. } => self.key_type is BoolList,
                    }
                }
            }
        }
